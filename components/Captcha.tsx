...
function Captcha({ onChange, captchaKey }: CaptchaProps) {
  ...
  return (
    <div className="p-2">
      <h2 className="text-3xl lg:text-6xl text-gray-900 font-Raleway font-extrabold uppercase">
        Captcha
      </h2>
      <p className="text-gray-900 lg:text-2xl font-Raleway uppercase font-extrabold">
        Selecciona todas las imágenes que son un Letrero!!! 🤖{' '}
      </p>
      <div className="flex justify-center py-4">
        <div className="bg-white grid grid-cols-3 gap-1 ">
          {imageLocation.map((imageUrl, index) => (
            <div
              key={index + imageUrl}
              className="relative min-h-[72px] md:mi-h-[152px] bg-white"
              onClick={() => toggleIndex(index)}
            >
              <picture>
                <source srcSet={imageUrl} type="image/jpg" />

                <img alt="captcha" draggable={false} height={152} src={imageUrl} width={165} />
              </picture>
