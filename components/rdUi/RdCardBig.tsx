...
function RdCardBig({ prefixtitle, title, attribution, excerpt, imgSrc, linkText, linkUrl, gradient, color }: RdCardProps) {
  return (
    <div
      className={clsx("lg:col-span-12 xl:col-span-12 rounded-3xl flex flex-col-reverse gap-4 md:flex-row justify-between items-center bg-gradient-to-br py-4", {
    ...
    >
      <div className='px-4 md:px-8 flex flex-col gap-y-4 justify-around md:w-1/2 md:h-full'>
        ...
        <p className={clsx("lg:text-lg", { "text-white": color === "light", "text-brand-black-900": color === "dark" })}>{excerpt} </p>
        ...
        {attribution ? (
          <div>
            <p className={clsx({ "text-white": color === "light", "text-brand-black-300": color === "dark" })}>
              Este trabajo se basa en&nbsp;
              <a href={attribution.baseonUrl} target='_blank' rel='noopener noreferrer'>
                <span
      ...
      <div className='md:flex-1 h-full flex justify-center items-center'>
        <Image src={imgSrc} objectFit='contain' objectPosition='center' alt='3D render cartoon car' className='w-full h-full rounded-3xl' />
      </div>
    </div>
  );
}
