...
import { robotCaptchaImage } from './captcha-images'
...
export default withIronSessionApiRoute(
  async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    const correctIndexes = req.session.captchaImages
      .map((path, index) => (path.includes('correct') ? index : -1))
    ...
    try {
      ...
      res
        .status(200)
        .json({ message: 'Message no send coz captcha is no correct ', captchaIsOK, send })
    } catch (error) {
